		...
		?>
			...
			<table class="table table-bordered">
				...
				foreach ($user->showData($newquery) as $value) {
					...
					<td><?=$value['email']; ?></td>
					<td><?=$value['level']?></td>
					<td><img src="../images/<?=$value['foto']?>" height="70px" width="auto"></td>
					<td>
						<a href="?page=user_edit&id=<?=$value['id']?>" title="edit"><span class="glyphicon glyphicon-edit"></span></a>
					</td>
					<td>
						<a href="?page=delete&user_id=<?php print($value['id']) ?>" onclick="return confirm('Anda yakin ingin menghapus data user <?php echo $value['nama']; ?> ?')" title="Hapus"><span class="glyphicon glyphicon-trash"></span></a>
					</td>
					</tr>
					<?php
					$no++;
